...
Class TabButton
    ...
    Private Property IsUISet As Boolean = False
    ...
    Overrides Sub SetButtonData(btnData As MLButtonData)
        ...
        If btnData IsNot Nothing AndAlso
            ...
            TabControlData.GetInstance.Button.AddButtonRow(
                TabControlData.GetInstance.Tab.FindById(MainTabControl.ConvertToTabId(GetParentTabIndex)),
                Me.Scancode,
                btnData.Text,
                btnData.IconFile,
                btnData.IconIndex,
    ...
    Function GetButtonRow() As TabControlData.ButtonRow
        Return TabControlData.GetInstance.Button.FindByTab_IDId(MainTabControl.ConvertToTabId(GetParentTabIndex), Me.Scancode)
    End Function

    Private Function GetParentTabItem(control As FrameworkElement) As TabItemControl
        Dim ctrl As FrameworkElement = control.Parent

        Try
            If Not (TypeOf ctrl Is TabItemControl) Then ctrl = GetParentTabItem(control.Parent)
        Catch ex As Exception

        End Try

        Return CType(ctrl, TabItemControl)
    End Function

    Function GetParentTabIndex() As Integer
        If Not DesignerProperties.GetIsInDesignMode(Me) Then
            Dim parentTabItem As TabItemControl = GetParentTabItem(Me)

            Return CType(parentTabItem.Parent, MainTabControl).Items.IndexOf(parentTabItem)
        End If

        Return 99
    End Function

    Function GetParentTabName() As String
        If Not DesignerProperties.GetIsInDesignMode(Me) Then
            Return GetParentTabItem(Me).Header
        End If

        Return ""
    End Function

    Overrides Sub SetVisibility()
        If Not DesignerProperties.GetIsInDesignMode(Me) Then
            If ConfigManager.AppConfig.HideEmptyTabButtons AndAlso (GetButtonRow() Is Nothing) Then
...
#Region "Button Events"
    ...
    Private Sub MButton_Loaded(sender As Object, e As RoutedEventArgs)
        If Not DesignerProperties.GetIsInDesignMode(Me) Then
            If Not IsUISet Then
                Me.Key = Keyboard.GetText(Me.Scancode)
                UpdateUI()
                IsUISet = True
            End If
        End If
